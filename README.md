...
`REFERENCE` can be combined with `OPTIONAL`: `FUNCTION process OPTIONAL REFERENCE items@`
...
**Scoping:** Functions can read global variables automatically. Variables assigned inside a function (including parameters) are local. Built-in color constants (e.g., `RED&`) are available in every scope. Labels inside functions are local and don't conflict with global labels. Function names shadow variable names — if you define `FUNCTION add#`, then `add#` always refers to the function.
...
FUNCTION showScore
  PRINT name$ + ": " + score#   ' reads globals automatically
END FUNCTION

showScore   ' Player 1: 100
```
