function parse(tokens, existingFunctions) {
  ...
  function parseStatement() {
    ...
    if (t.type === 'KEYWORD' && t.value === 'DRAWSPRITE') {
      ...
      return { type: 'drawsprite', sprite: resolved.SPRITE, x: resolved.X, y: resolved.Y, flipH: resolved.FLIPH || null, flipV: resolved.FLIPV || null, scale: resolved.SCALE || null, scaleX: resolved.SCALEX || null, scaleY: resolved.SCALEY || null, angle: resolved.ANGLE || null, line: dt.line };
    }
  ...
  const BUILTIN_KEYWORD_PARAMS = {
    ...
    READSCREEN: [{ name: 'ROW', required: true }, { name: 'COLUMN', required: true }],
    ...
    BOXCOLLIDES: [
      { name: 'X1', required: true },
      { name: 'Y1', required: true },
      { name: 'W1', required: true },
      { name: 'H1', required: true },
      { name: 'X2', required: true },
      { name: 'Y2', required: true },
      { name: 'W2', required: true },
      { name: 'H2', required: true },
    ],
    GROUP3D: [],
